...
public abstract class NutritionFoodActivity extends NutritionTodayActivity {
    ...
    protected String basisSuffix(NutritionData.Food food) {
        if (food.isPerServing()) return "/份";
        return food.isPerMilliliter() ? "/100ml" : "/100g";
    }
    ...
    @Override protected void showFoodPicker(String meal) {
        ...
        EditText search = input("搜索食物", false);
        ...
        AlertDialog dialog = new AlertDialog.Builder(this)
                ...
                .create();
        Runnable render = () -> renderFoodRows(list,
                filterFoods(search.getText().toString(), String.valueOf(category.getSelectedItem())),
                meal, dialog);
    ...
    @Override protected void showAmountDialog(NutritionData.Food food, String meal) {
        ...
        android.widget.TextView preview = muted("");
        wrap.addView(text("餐次", 13, true));
        ...
        Runnable update = () -> {
            double a = parse(amount);
            double r = food.ratio(a);
            preview.setText(one(food.kcal * r) + " kcal · 蛋白" + one(food.protein * r)
                    + "g · 脂肪" + one(food.fat * r) + "g · 碳水" + one(food.carb * r) + "g");
        };
        amount.addTextChangedListener(new SimpleWatcher(update));
        update.run();
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("加入记录", (d, w) -> {
                    ...
                    e.date = selectedDate.toString();
                    e.meal = String.valueOf(mealSpinner.getSelectedItem());
                    e.name = food.name;
                    e.amount = a;
                    e.amountUnit = food.amountUnit();
                    e.kcal = food.kcal * r;
                    e.protein = food.protein * r;
                    e.fat = food.fat * r;
                    e.carb = food.carb * r;
                    e.fiber = food.fiber * r;
                    e.sodium = food.sodium * r;
                    entries.add(e);
    ...
    @Override protected void showFoodLibrary() {
        ...
        EditText search = input("搜索米饭、鸡蛋、牛奶……", false);
        Spinner category = spinner(categories(), "全部");
        LinearLayout list = new LinearLayout(this);
        list.setOrientation(LinearLayout.VERTICAL);
        ...
        Runnable render = () -> renderLibraryRows(list,
                filterFoods(search.getText().toString(), String.valueOf(category.getSelectedItem())));
        search.addTextChangedListener(new SimpleWatcher(render));
        category.setOnItemSelectedListener(new SimpleItemSelected(render));
        render.run();
    ...
    private void renderLibraryRows(LinearLayout list, List<NutritionData.Food> foods) {
        ...
        for (NutritionData.Food f : foods) {
            ...
            detail.addView(muted(f.category + " · " + one(f.kcal) + " kcal" + basisSuffix(f)
                    + " · 蛋白" + one(f.protein) + "g · 脂肪" + one(f.fat)
                    + "g · 碳水" + one(f.carb) + "g"));
            Button add = button("＋");
    ...
    protected static final class SimpleWatcher implements TextWatcher {
        ...
        SimpleWatcher(Runnable a) { action = a; }
    ...
    protected static final class SimpleItemSelected implements AdapterView.OnItemSelectedListener {
        private final Runnable action;
        SimpleItemSelected(Runnable a) { action = a; }
